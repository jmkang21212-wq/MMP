...
알림은 todo 하나당 한 번만 갑니다. 리뷰어 지정, MR 본문 멘션, 댓글 멘션은 각각 별도의 todo로 들어오므로, 이미 알린 MR이라도 새로 멘션되면 다시 알립니다. todo 없이 리뷰어 지정만으로 잡힌 MR은 MR당 한 번만 갑니다. 알림 여부는 리뷰 상태와 별도로 기록되므로, 감시 프로세스가 먼저 확인해도 세션에서 `is_new`는 그대로 유지됩니다.

감시 프로세스는 감지와 알림만 합니다. 확인 이모지, diff 조회, 리뷰 작성, 댓글 게시는 하지 않습니다. 리뷰를 쓰려면 모델이 필요한데 이 프로세스에는 없습니다. 알림을 받으면 세션에서 이어서 진행하세요.

### 열린 MR 사이의 충돌 미리 보기

승인하기 전에, 같은 프로젝트에서 열려 있는 다른 MR과 하나씩 가상 병합해 봅니다. 워킹 트리·인덱스·브랜치를 건드리지 않고 트리 객체만 만들어 읽습니다.

```powershell
node scripts/cross-mr-check.mjs --site ssafy --project 1439837 --mr 184 --repo C:\경로\체크아웃
```

| 옵션 | 기본값 | 설명 |
|---|---|---|
| `--mr <iid>` | 필수 | 검사할 대상 MR |
| `--project <id>` | 필수 | 숫자 ID 또는 프로젝트 경로 |
| `--site <이름>` | 등록된 사이트가 하나면 자동 | GitLab 인스턴스 |
| `--repo <경로>` | 현재 디렉터리 | 로컬 체크아웃. 여기서 git을 실행합니다 |
| `--limit <수>` | `50` | 비교할 열린 MR 수의 상한. 1~100 |
| `--no-fetch` | | 브랜치를 받아오지 않습니다. head가 로컬에 없으면 실행 불가로 끝냅니다 |

**잡는 것이 두 종류입니다.**

- `[conflict]` — git이 충돌로 보고하는 것. 병합할 때 사람이 반드시 보게 되므로 상대적으로 덜 위험합니다.
- `[silent]` — **git이 깨끗이 병합했는데 결과가 깨진 것.** 두 브랜치가 같은 속성 키를 파일의 서로 다른 줄에 넣으면 텍스트 충돌이 아니라 둘 다 살아남고, 뒤에 오는 값이 조용히 이깁니다. 이 검사가 존재하는 이유가 이쪽입니다.

현재 `[silent]` 판정은 두 가지입니다 — `.properties`의 중복 키, 같은 Flyway 번호를 주장하는 서로 다른 마이그레이션. **이 목록은 겪어 봐야 늘어납니다.** 새 유형을 만나면 [src/merge-inspection.js](src/merge-inspection.js)에 판정을 더하고 [test/cross-mr-check.test.js](test/cross-mr-check.test.js)에 실제 사례를 고정합니다. 판정을 순수 함수로 떼어 둔 것은 저장소 두 개를 준비하지 않고도 규칙을 검증하기 위해서입니다.

MCP 도구로 노출하지 않았습니다. 서버 프로세스는 GitLab 토큰과 웹훅 URL을 들고 있어서, 호출자가 지정한 경로에서 명령을 실행하는 권한을 같은 프로세스에 두지 않습니다. git은 이 스크립트에서만 돌고 `src/`는 API만 담당합니다.

종료 코드는 `0` 깨끗함, `1` 발견됨, `2` 실행 불가입니다. `git merge-tree --write-tree`를 쓰므로 git 2.38 이상이 필요합니다.

### 지적 대장

리뷰에서 무엇을 막았는지 세션 밖으로 남깁니다. 재리뷰가 답해야 할 질문은 하나입니다 — **작성자가 그 지적이 가리킨 파일을 건드렸는가.** MR의 discussions에는 문장이 남지만 "내가 어느 파일을 말한 거였지"를 문장에서 복원하는 건 추측입니다. 지적을 올린 시점의 head sha와 대상 경로를 함께 기록해 두면 그 추측이 diff 한 줄로 바뀝니다.

| 도구 | 하는 일 |
|---|---|
| `gitlab_finding_record` | MR·head sha·요약·대상 경로·차단 여부를 기록 |
| `gitlab_finding_list` | 해당 MR의 열린 지적. `include_resolved`로 닫힌 것까지 |
| `gitlab_finding_resolve` | 반영을 확인한 뒤 닫고, 닫은 head sha를 남김 |

재리뷰 절차는 이렇습니다.

```
git diff --name-only <finding.headSha> <새 head>
```

결과가 `finding.paths`와 겹치지 않으면 **고쳤다고 했는데 그 파일을 안 건드린 것**이고, 그대로 말할 수 있습니다.

`paths`는 비워도 됩니다. MR 본문이 실제 변경과 어긋난다는 식의 지적은 파일에 매이지 않습니다. 대신 그런 지적은 diff로 확인할 수 없다는 뜻이기도 합니다.

git을 읽지 않습니다. 기억만 하고 diff는 호출자가 합니다.
